...
FRAMES: list[tuple[str, str, re.Pattern]] = [
    ...
    # جذر «استثن» بكل صوره: يستثني، تستثن، استثنت، المستثنين، الاستثناء.
    # الاقتصار على «يستثن» كان يفوّت الادعاء الذي يعكس الاستثناء فعلًا، فيُحال
    # للتحكيم بلا سبب — والإحالة بلا سبب تُكلف وقت مدقق في كل مذكرة.
    ("استثناء", "عدم سريان", re.compile(
        r"لا\s+تسري|لا\s+تنطبق|لا\s+يسري|استثن|[يتم]ستثن|"
        r"مع\s+عدم\s+الإخلال|فيما\s+عدا")),
