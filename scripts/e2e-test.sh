...
# ── 6ز. تدقيق الحقول الحرجة ──────────────────────────────────────────
step "6ز. تدقيق الحقول الحرجة في مستند العميل"
cat > "$TMP/mustanad.txt" <<'DOC'
إشعار إنهاء خدمة
الموافق 14 مارس 2024، ويسري اعتبارًا من 14/04/2024.
الأجر الأساسي 620.50 دينارًا، وبدل السكن 150 دينارًا، والمجموع 800 دينار.
الرقم الشخصي 879912345.
DOC
python3 tools/documents/verify_fields.py "$TMP/mustanad.txt" \
  --out "$TMP/tadqiq.md" >"$TMP/vf.log" 2>&1
VF=$?
if grep -q "ثلاث (فلس)" "$TMP/vf.log"; then
  ok "رُصد مبلغ بخانتين عشريتين في الدينار"
else
  bad "لم يُرصد المبلغ ناقص الخانة"; tail -6 "$TMP/vf.log" | sed 's/^/      /'
fi
if grep -q "لا يتطابقان" "$TMP/vf.log"; then
  ok "  ورُصد مجموع لا يساوي مفرداته"
else
  bad "لم يُرصد المجموع المخالف"
fi
if grep -q "ليست تاريخًا ممكنًا" "$TMP/vf.log"; then
  ok "  ورُصد رقم شخصي تاريخه مستحيل"
else
  bad "لم يُرصد الرقم الشخصي المستحيل"
fi
# الورقة الفارغة ليست موافقة
python3 tools/documents/verify_fields.py "$TMP/mustanad.txt" \
  --confirm "$TMP/tadqiq.md" >"$TMP/vfc.log" 2>&1
if [ "$?" -eq 1 ] && grep -q "بلا مقابلة" "$TMP/vfc.log"; then
  ok "  وورقة بلا مقابلة تُرفض — الفراغ ليس موافقة"
else
  bad "قُبلت ورقة تدقيق فارغة"
fi
