...
### ثم دقّق الحقول الحرجة

الثقة تقيس **شكل** النص لا صحته: «٣٠ يومًا» تُقرأ «٣٧ يومًا» فتبقى الجملة فصيحة
والثقة عالية والمدة خطأ. فخطوة ثانية تحصر الشك في الأرقام التي تدور عليها القضية:

```bash
python3 tools/documents/verify_fields.py عقد.pdf --out cases/118/tadqiq.md
# املأ سطر «بالأصل» في كل حقل بما تقرؤه في المستند نفسه، ثم:
python3 tools/documents/verify_fields.py عقد.pdf --confirm cases/118/tadqiq.md
```

تقابل قراءتين مستقلتين للملف نفسه، وتفحص التماسك الداخلي (تاريخ مستحيل، مجموع
لا يساوي مفرداته، مدة تناقض حدّيها، رقم شخصي تاريخه غير ممكن، دينار بخانتين
عشريتين بدل ثلاث)، ثم تطلب مقابلة الحقول وحدها بالأصل.

| الرمز | المعنى |
|---|---|
| `0` | لا مقابلة لازمة — مصدر حتمي (DOCX/HTML/نص) وحقول متماسكة |
| `3` | تحتاج مقابلة بالأصل — أُنتجت ورقة التدقيق |
| `1` | ورقة ناقصة المقابلة — الفراغ ليس موافقة |

**ولا يُغني عن هذا نموذجٌ يراجع النص:** الخانة الخاطئة اختفت منه.
